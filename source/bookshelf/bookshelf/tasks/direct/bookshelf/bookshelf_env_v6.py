...
"""Bookshelf-Direct-v6 environment.

V6 is v5 with strict residual-RL action composition:

    delta_final = delta_nominal + delta_policy_residual

The nominal term is a conservative geometric insertion/push controller. PPO
still outputs bounded Cartesian residuals, and the final command is executed
through the same differential IK path as v5.
"""
...
from .bookshelf_env_cfg_v6 import BookshelfEnvCfg
...
class BookshelfEnv(BookshelfEnvV5):
    """V5 task with nominal geometric controller plus learned residual actions."""
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        if (
            ...
            and not bool(getattr(self.cfg, "debug_disable_nominal_release", False))
        ):
    ...
    def _nominal_cartesian_delta(self, mode: torch.Tensor) -> torch.Tensor:
        ...
        target_tool_pos, target_yaw, target_pitch = self._planned_tool_release_pose()
        push_tool_pos, _, _ = self._planned_tool_release_pose(inside_fraction=0.9)
        target_tool_pos = torch.where(push_mask.unsqueeze(-1), push_tool_pos, target_tool_pos)

        tool_pos = self._ee_tool_pos_env()
        _, ee_quat_b = self._ee_pose_in_base()
        _, ee_pitch_b, ee_yaw_b = math_utils.euler_xyz_from_quat(ee_quat_b)

        pos_delta = float(self.cfg.nominal_plan_position_gain) * (target_tool_pos - tool_pos)
        dyaw = float(self.cfg.nominal_plan_yaw_gain) * _wrap_to_pi(target_yaw - ee_yaw_b)
        dpitch = float(self.cfg.nominal_plan_pitch_gain) * _wrap_to_pi(target_pitch - ee_pitch_b)
        ...
        yaw_ready = torch.abs(_wrap_to_pi(target_yaw - ee_yaw_b)) < float(self.cfg.nominal_plan_yaw_thresh)
        pitch_ready = torch.abs(_wrap_to_pi(target_pitch - ee_pitch_b)) < float(self.cfg.nominal_plan_pitch_thresh)
        orientation_ready = yaw_ready & pitch_ready
        if bool(getattr(self.cfg, "nominal_plan_orientation_first", True)) and not bool(
            getattr(self.cfg, "debug_position_only_target_ee", False)
        ):
            pos_delta = torch.where(orientation_ready.unsqueeze(-1), pos_delta, torch.zeros_like(pos_delta))

        dx_limit = torch.full((self.num_envs,), float(self.cfg.nominal_insert_dx), device=self.device, dtype=torch.float32)
        dx_limit = torch.where(push_mask, torch.full_like(dx_limit, float(self.cfg.nominal_push_dx)), dx_limit)
        nominal[:, 0] = torch.clamp(pos_delta[:, 0], -dx_limit, dx_limit)
        nominal[:, 1] = torch.clamp(pos_delta[:, 1], -float(self.cfg.nominal_dy_limit), float(self.cfg.nominal_dy_limit))
        nominal[:, 2] = torch.clamp(pos_delta[:, 2], -float(self.cfg.nominal_dz_limit), float(self.cfg.nominal_dz_limit))
        nominal[:, 3] = torch.clamp(
            dyaw, -float(self.cfg.nominal_dyaw_limit), float(self.cfg.nominal_dyaw_limit)
        )
        nominal[:, 4] = torch.clamp(
            dpitch, -float(self.cfg.nominal_dpitch_limit), float(self.cfg.nominal_dpitch_limit)
        )
    ...
    def _apply_action(self) -> None:
        ...
        delta = self._nominal_cartesian_delta(mode) + residual
