...
"""Bookshelf-Direct-v2: yaw-enabled planar wrench control, narrow slot with walls, contact-rich insertion."""
...
from .bookshelf_env_cfg_v2 import BookshelfEnvCfg
...
class BookshelfEnv(DirectRLEnv):
    ...
    def _contact_wrench_world(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Some Isaac Lab versions expose net_torques_w; fall back to zeros if absent.
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        mz = torch.abs(t[:, 2])
        # Penalize only "high" contact; mild contact is expected/necessary in insertion.
        contact_pen = self.cfg.contact_penalty_scale * torch.clamp((fn - 10.0) / 50.0, min=0.0, max=4.0)
        torque_pen = self.cfg.torque_penalty_scale * torch.clamp((mz - 0.3) / 3.0, min=0.0, max=4.0)
        ...
        # Dense alignment bonus (prevents "wandering"): reward being aligned even before progress.
        align_bonus = self.cfg.alignment_bonus_scale * gate
        ...
        # Terminal failure penalties (so "exit shelf quickly" is never optimal).
        ...
        self.extras["log"]["contact_mz_abs"] = mz.mean()
    ...
    def _reset_idx(self, env_ids):
        ...
        qyaw = torch.stack((torch.cos(0.5 * yaw), torch.zeros_like(yaw), torch.zeros_like(yaw), torch.sin(0.5 * yaw)), dim=-1)
        qstand = torch.tensor(self.cfg.book_standing_quat, device=self.device, dtype=book_default_state.dtype).expand(
            len(env_ids), 4
        )
        ...
        self._prev_yaw_err[env_ids] = torch.abs(yaw)
