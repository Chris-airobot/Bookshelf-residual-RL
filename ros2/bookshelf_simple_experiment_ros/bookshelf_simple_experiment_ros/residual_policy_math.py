"""Exact portable residual actor and INSERT controller math.

Source-equivalent to the verified shadow-policy helpers; runtime stays isolated.
"""
...
def compute_policy_nominal_delta(
    ...
    raw = _vector(raw_metrics, POLICY_OBSERVATION_SIZE, "raw_metrics")
    if not math.isclose(float(raw[0]), 0.0, abs_tol=1.0e-4):
        raise ValueError(
            f"unsupported policy mode observation {float(raw[0]):.4f}; "
            "this one-step controller supports INSERT only"
        )
...
def scale_residual_action(
    ...
    action = np.clip(
        _vector(policy_action, POLICY_ACTION_SIZE, "policy_action"), -1.0, 1.0
    )
...
class NumpyActorBundle:
    """Portable deterministic PPO actor and VecNormalize statistics."""

    SCHEMA_VERSION = 1
    REQUIRED_KEYS = {
        "schema_version", "observation_size", "action_size", "activation",
        "obs_mean", "obs_var", "obs_epsilon", "obs_clip", "action_low",
        "action_high", "policy_0_weight", "policy_0_bias", "policy_1_weight",
        "policy_1_bias", "action_weight", "action_bias", "metadata_json",
    }
    ...
    def __init__(self, path):
        ...
        if int(values["schema_version"].item()) != self.SCHEMA_VERSION:
        ...
        self.obs_mean = _vector(values["obs_mean"], 12, "obs_mean")
        self.obs_var = _vector(values["obs_var"], 12, "obs_var")
        ...
        self.action_low = _vector(values["action_low"], 6, "action_low")
        self.action_high = _vector(values["action_high"], 6, "action_high")
    ...
    def normalize_observation(self, observation) -> np.ndarray:
        observation = _vector(observation, 12, "observation")
    ...
    def predict(self, observation) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        hidden = np.maximum(self.policy_0_weight @ normalized + self.policy_0_bias, 0.0)
        hidden = np.maximum(self.policy_1_weight @ hidden + self.policy_1_bias, 0.0)
