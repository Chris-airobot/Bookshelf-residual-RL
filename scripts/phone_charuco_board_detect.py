...
# Printed board:
# calib.io_charuco_210x320_12x8_25_18_DICT_4X4_7047.pdf
# Target Type: ChArUco
# Board: 210 mm x 320 mm
# Rows: 12, Columns: 8
# Checker width: 25 mm
...
DEFAULT_SQUARE_LENGTH = 0.025
DEFAULT_MARKER_LENGTH = 0.018
DEFAULT_APRILTAG_IDS = "0,1,2"
DEFAULT_APRILTAG_LENGTH = 0.019
DEFAULT_BOOK_DEPTH = 0.126
DEFAULT_BOOK_HEIGHT = 0.186
DEFAULT_BOOK_THICKNESS = 0.026
DEFAULT_BOOK_CORNER_BOARD = "0,0,0"
# Isaac book convention:
#   +X_book = depth / insertion length
#   +Y_book = height
#   +Z_book = thickness
# Current physical calibration placement:
#   the book spine side touches the ChArUco plane. From the annotated camera
#   image, height follows board +Y and thickness follows board +X. To keep a
#   right-handed Isaac-style book frame, depth follows board -Z.
...
def main():
    ...
    parser.add_argument("--min_corners", type=int, default=4, help="Minimum ChArUco corners needed for pose.")
    parser.add_argument("--axis_length", type=float, default=0.06, help="Displayed board axis length in meters.")
    parser.add_argument(
        "--apriltag_dictionary",
        choices=sorted(APRILTAG_DICT_NAMES),
        default="tag36h11" if "tag36h11" in APRILTAG_DICT_NAMES else None,
        help="AprilTag dictionary for book markers.",
    )
    parser.add_argument(
        "--apriltag_ids",
        default=DEFAULT_APRILTAG_IDS,
        help="Comma-separated AprilTag IDs to detect.",
    )
    parser.add_argument(
        "--apriltag_length",
        type=float,
        default=DEFAULT_APRILTAG_LENGTH,
        help="AprilTag black/coded square side length in meters.",
    )
    parser.add_argument(
        "--book_depth",
        type=float,
    ...
    apriltag_ids = parse_int_list(args.apriltag_ids)
    apriltag_dictionary = None
    if apriltag_ids:
        if args.apriltag_dictionary is None:
            raise RuntimeError(
                "This OpenCV install does not expose DICT_APRILTAG_36h11. "
                "Use an OpenCV contrib build with AprilTag dictionary support."
            )
        apriltag_dictionary = cv2.aruco.getPredefinedDictionary(
            APRILTAG_DICT_NAMES[args.apriltag_dictionary]
        )
    ...
    last_print = 0.0
    pose_count = 0
    tag_samples = {tag_id: [] for tag_id in apriltag_ids}
    ...
    while True:
        ...
        apriltag_marker_ids = None
        if apriltag_dictionary is not None:
            apriltag_corners, apriltag_marker_ids, _ = detect_markers(
                gray, apriltag_dictionary, detector_params
            )
        board_found = False
        num_charuco = 0
        tag_summaries = []
        ...
        if marker_ids is not None:
            ...
            if retval >= args.min_corners:
                ...
                if ok_pose:
                    board_found = True
                    pose_count += 1
                    cv2.drawFrameAxes(frame, K, D, rvec, tvec, args.axis_length)
                    ...
                    cv2.drawFrameAxes(frame, K, D, book_rvec, book_tvec, args.axis_length * 0.8)

                    if apriltag_marker_ids is not None:
                        cv2.aruco.drawDetectedMarkers(frame, apriltag_corners, apriltag_marker_ids)
                        flat_tag_ids = apriltag_marker_ids.reshape(-1)
                        for tag_id in apriltag_ids:
                            ...
                            tag_rvecs, tag_tvecs, _ = cv2.aruco.estimatePoseSingleMarkers(
                                [apriltag_corners[idx]],
                                args.apriltag_length,
                                K,
                                D,
                            )
                            ...
                            tag_draw_len = min(args.apriltag_length, args.axis_length * 0.45)
                            cv2.drawFrameAxes(frame, K, D, tag_rvec, tag_tvec, tag_draw_len)
        ...
        if args.collect_samples > 0:
            counts_text = ",".join(f"{tag_id}:{len(tag_samples[tag_id])}" for tag_id in apriltag_ids)
            if apriltag_ids and all(len(tag_samples[tag_id]) >= args.collect_samples for tag_id in apriltag_ids):
                break
        else:
            counts_text = ""
        ...
        status = (
            ...
            f"tags={len(tag_summaries)}/{len(apriltag_ids)} poses={pose_count}"
        )
        if counts_text:
        ...
        if not args.no_preview:
            cv2.imshow("phone charuco board detect", frame)
    ...
    if args.collect_samples > 0:
        summaries = {}
        save_arrays = {
            "tag_ids": np.asarray(apriltag_ids, dtype=np.int64),
